...
async function handleAddBook(e) {
    ...
    customBooks.push({
        ...
        coverImg: document.getElementById('add-cover-img').value.trim() || null,
...
async function adminDeleteUser(username) {
    if (!confirm('Hapus user "' + username + '"? Data ini tidak bisa dikembalikan.')) return;
    var users = getUsers().filter(function(u) { return u.username !== username; });
    await saveUsers(users);
    // Hapus juga data pinjaman user ini
    var borrows = getBorrows().filter(function(b) { return b.username !== username; });
