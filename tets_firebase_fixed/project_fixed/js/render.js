function renderHome() {
    ...
    grid.innerHTML = filtered.map(function(book, i) {
        ...
        var badge = borrowed ? '<span class="badge badge-no">Dipinjam</span>' : '<span class="badge badge-ok">Tersedia</span>';
        if (hasPdf) badge += ' <span class="badge badge-pdf">PDF</span>';
        else badge += ' <span class="badge" style="background:#FEF3C7;color:#92400E;">Belum ada PDF</span>';
        return '<div class="book-card anim-fade" style="animation-delay:' + (i * .04) + 's" onclick="navigate(\'detail\',' + book.id + ')">' +
            '<div style="background:linear-gradient(135deg,' + book.color1 + ',' + book.color2 + ');aspect-ratio:3/4;display:flex;flex-direction:column;justify-content:flex-end;padding:1.125rem;position:relative;overflow:hidden;">' +
                '<div style="position:absolute;top:.5rem;right:.5rem;">' + badge + '</div>' +
...
function renderDetail(bookId) {
    ...
    if (isAdmin) {
        ...
        actionBtn = isMyBorrow
            ...
            : '<button class="btn btn-ghost" disabled style="opacity:.5;cursor:not-allowed;"><i class="fas fa-lock"></i> Sedang Dipinjam</button>';
    } else {
    ...
    var badges = borrowed ? '<span class="badge badge-no">Dipinjam oleh ' + borrowInfo.username + '</span>' : '<span class="badge badge-ok">Tersedia</span>';
    if (hasPdf) badges += ' <span class="badge badge-pdf">PDF</span>';
    else badges += ' <span class="badge" style="background:#FEF3C7;color:#92400E;">Belum ada PDF</span>';
    ...
    '<div class="anim-fade" style="display:flex;gap:2rem;flex-wrap:wrap;">' +
        ...
        '<div style="flex:1;min-width:260px;">' +
            ...
            '<div style="display:flex;gap:.75rem;flex-wrap:wrap;">' + readBtn + actionBtn + '</div>' +
        '</div>' +
    '</div>';
}
...
function renderAdmin() {
    ...
    var borrowTable = activeBorrows.length === 0
        ...
        : '<div style="overflow-x:auto;"><table class="atable"><thead><tr><th>Buku</th><th>Peminjam</th><th>Tanggal</th><th>Aksi</th></tr></thead><tbody>' +
            activeBorrows.map(function(b) {
                var book = getBookById(b.bookId);
                return '<tr><td style="font-weight:600;">' + (book ? book.title : '?') + '</td><td>' + b.username + '</td><td>' + formatDate(b.borrowDate) + '</td>' +
                '<td><button class="btn btn-danger btn-sm" onclick="adminForceReturn(' + b.bookId + ')"><i class="fas fa-undo"></i> Kembalikan</button></td></tr>';
    ...
    var userTable = users.length === 0
        ...
        : '<div style="overflow-x:auto;"><table class="atable"><thead><tr><th>Nama</th><th>Username</th><th>Terdaftar</th></tr></thead><tbody>' +
            users.map(function(u) { return '<tr><td style="font-weight:600;">' + u.name + '</td><td>' + u.username + '</td><td>' + formatDate(u.createdAt) + '</td></tr>'; }).join('') +
            '</tbody></table></div>';
    ...
    var bookTable = getAllBooks().length === 0
        ...
        : '<div style="overflow-x:auto;"><table class="atable"><thead><tr><th>Judul</th><th>Penulis</th><th>Kategori</th><th>PDF</th><th>Aksi</th></tr></thead><tbody>' +
            getAllBooks().map(function(b) {
                return '<tr><td style="font-weight:600;">' + b.title + '</td><td>' + b.author + '</td><td>' + b.category + '</td>' +
